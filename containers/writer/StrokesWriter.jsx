...
let createHandlers = function(dispatch) {
  let onAddChars = function(chars) {
    if (chars.length < 1) {
      ...
      dispatch(writerActions.sendCharRequest())
      dispatch(writerActions.requestChar(chars[0]))
    }
  }
  ...
  let onErrorsDismiss = function() {
    dispatch(deleteErrors(WriterActionTypes.DELETE_ERRORS))
  }

  let onMessagesDismiss = function() {
    dispatch(deleteMessages(WriterActionTypes.DELETE_MESSAGES))
  }
  ...
  render() {
    return (
      <Container>
        ...
        <Loader show={ !this.props.writer.get('writerLoaded') } message='loading'>
        <Row>
